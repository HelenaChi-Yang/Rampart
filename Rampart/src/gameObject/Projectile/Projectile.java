...
public abstract class Projectile extends GameObject {
    ...
    public void towardTarget(Actor monster) {
        ...
        fireAngle = Math.atan2(getDistanceY(),getDistanceX());      //計算發射角度，這邊輸出是arc

        velocityX = velocity * Math.cos(fireAngle);     //速度分量
        velocityY = -velocity * Math.sin(fireAngle);     //速度分量，Y軸要加負號

        velocityUpdateCountX = (int) Math.round(Global.UPDATE_TIMES_PER_SEC / velocityX);       //X分量計算，速度慢就多次更新才移動1Pixel
        if (velocityUpdateCountX == 0){      //在更新的地方是updateCount % velocityUpdateCount，所以velocityUpdateCount要>0
        ...
        velocityUpdateCountY = (int) Math.round(Global.UPDATE_TIMES_PER_SEC / velocityY);       //X分量計算，速度慢就多次更新才移動1Pixel
        if (velocityUpdateCountY == 0){      //在更新的地方是updateCount % velocityUpdateCount，所以velocityUpdateCount要>0
